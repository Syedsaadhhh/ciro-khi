...
class SifterAgent:
    ...
    SYSTEM_PROMPT = """You are a crisis classification AI for Karachi, Pakistan.
You ingest raw text data streams and classify inputs into two seasonal matrices: Monsoon Flooding or Summer Heatwave & Power Grid Failure.
You analyze weather data, traffic congestion, and social media signals to detect the crisis type and severity.

Given the signal data, you must:
1. Determine if a real event is occurring
2. Assign a severity score (1-10)
3. Assign a confidence score (0.0-1.0)
4. Classify the event type as either "monsoon_flooding" or "summer_heatwave_power_failure"
    ...
    def __init__(self):
        ...
        if settings.gemini_api_key:
            try:
                ...
                logger.info("sifter_gemini_ready", model=self.model)  # ADD THIS
            except Exception as e:
                logger.warning("sifter_gemini_init_failed", error=str(e))  # ADD THIS
        else:
            logger.warning("sifter_gemini_skipped", reason="no api key")  # ADD THIS
            
            
    async def run(self, signal_input: SignalInput, state: WorkflowState) -> IncidentModel | None:
        state.current_agent = "sifter"
        state.add_trace("Sifter", f"Starting signal analysis for {signal_input.location}")

        # Step 1: Gather signals
        weather = await get_weather_data(signal_input.location)
        if signal_input.weather_data:
            weather.update(signal_input.weather_data)
        if signal_input.precipitation_mm is not None:
            weather["rainfall_mm"] = signal_input.precipitation_mm
            if signal_input.precipitation_mm >= 50:
                weather["intensity"] = "extreme"
            elif signal_input.precipitation_mm >= 30:
                weather["intensity"] = "heavy"
            elif signal_input.precipitation_mm >= 10:
                weather["intensity"] = "moderate"
            else:
                weather["intensity"] = "light"

        traffic = await get_traffic_data(signal_input.location)
        if signal_input.traffic_data:
            traffic.update(signal_input.traffic_data)
        ...
        state.add_trace("Sifter", f"Weather: {weather['intensity']} rainfall ({weather['rainfall_mm']}mm)")
        state.add_trace("Sifter", f"Traffic: congestion level {traffic['congestion_level']}/10, {traffic['incidents']} incidents")
        state.add_trace("Sifter", f"Social: {social['flood_signal_count']} flood signals (score: {social['aggregate_score']})")

        # Step 2: Fast false-alarm check
        if self._is_obvious_false_alarm(weather, traffic, social):
            state.add_trace("Sifter", "Signal strength below threshold. Classifying as false alarm.")
            logger.info("sifter_false_alarm", location=signal_input.location)
            return None
        ...
        if incident:
            state.add_trace("Sifter", f"Incident detected: severity={incident.severity}, confidence={incident.confidence}")
            state.sifter_output = incident.model_dump()
            logger.info("sifter_incident_created",
  ...
  "reasoning": "<brief reasoning>"
        ...
        try:
            ...
            if data.get("is_false_alarm"):
                state.add_trace("Sifter", f"Gemini classified as false alarm: {data.get('reasoning', '')}")
                return None

            state.add_trace("Sifter", f"Gemini reasoning: {data.get('reasoning', 'N/A')}")
            ...
            state.add_trace("Sifter", f"Gemini unavailable, using deterministic fallback: {str(e)}")
