...
class StrategistAgent:
    """
    Validates sifter incidents via cross-source checks, performs impact
    analysis, estimates population and delay, generates rerouting plans.
    """

    SYSTEM_PROMPT = """You are a Seasonal Multi-Crisis response strategist for Karachi.
Given a detected incident (either monsoon_flooding or summer_heatwave_power_failure), your job is to:
1. Validate whether the incident is real using multi-source evidence
2. Estimate the affected population
3. Determine if traffic rerouting is needed (for floods: calculate alternative vehicle routing; for summer heatwaves: dynamically map power grid load-shedding blackouts and coordinate ambulance routing to public cooling camps)
...
Respond ONLY with valid JSON. No markdown. No explanation."""
    ...
    async def run(self, incident: IncidentModel, state: WorkflowState) -> PlanModel | None:
        ...
        state.add_trace("Strategist", f"Analyzing incident at {incident.location} (severity: {incident.severity})")

        # Step 1: Cross-validate with fresh data
        is_valid, rejection_reason = await self._validate_incident(incident, state)

        if not is_valid:
            state.add_trace("Strategist", f"Incident rejected: {rejection_reason}")
            logger.info("strategist_rejected_incident",
        ...
        state.add_trace("Strategist", "Running geofence and population estimation")
        ...
        if reroute_needed:
            state.add_trace("Strategist", f"Traffic congestion {congestion}/10 requires rerouting")
            routes = get_alternative_routes(incident.location, incident.severity)
            state.add_trace("Strategist", f"Generated {len(routes)} alternative routes")

        # Step 4: Priority classification
        priority = self._classify_priority(incident.severity)
        state.add_trace("Strategist", f"Incident priority: {priority.upper()}")
        ...
        state.strategist_output = plan.model_dump()
        state.add_trace("Strategist", f"Plan complete: {estimated_population:,} people affected, delay {delay_minutes}min")

        logger.info("strategist_plan_created",
            incident_id=incident.incident_id,
            priority=priority,
    ...
    async def _validate_incident(
        ...
        """
        state.add_trace("Strategist", "Cross-validating with fresh data sources")
        ...
        state.add_trace("Strategist", f"Validation passed with {len(incident.signal_sources)} confirmed sources")
        return True, ""
