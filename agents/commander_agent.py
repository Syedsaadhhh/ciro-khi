    ...
    def __init__(self):
        ...
        if settings.gemini_api_key:
            try:
                ...
                logger.info("commander_gemini_ready", model=self.model)
            except Exception as e:
                logger.warning("commander_gemini_init_failed", error=str(e))
        else:
            logger.warning("commander_gemini_skipped", reason="no api key")
...
Write only the alert message. Nothing else."""
        ...
        try:
            ...
            logger.info("commander_gemini_alert_generated", location=incident.location)
            return message
        except Exception as e:
            logger.warning("commander_gemini_alert_failed", error=str(e))
            return fallback

    async def run(
        self, incident: IncidentModel, plan: PlanModel, state: WorkflowState
    ) -> ActionModel:
        state.current_agent = "commander"
        state.add_trace("Commander", f"Generating actions for incident {incident.incident_id[:8]}")
        ...
        if smart_message != alert_obj["message"]:
            state.add_trace("Commander", f"Gemini alert: {smart_message}")
        else:
            state.add_trace("Commander", f"Alert created: {alert_obj['severity_label']} via {', '.join(alert_obj['channels'])}")
        ...
        if incident.severity >= 7:
            ...
            state.add_trace("Commander", f"Emergency ticket dispatched: {ticket_id}")
        ...
        state.add_trace("Commander", "Live dashboard update queued via WebSocket")
        ...
        state.commander_output = action.model_dump()
        state.add_trace("Commander", f"All actions complete: {', '.join(actions)}")

        logger.info("commander_actions_complete",
            incident_id=incident.incident_id,
